...
exports.addSlider = catchAsync(async (req, res, next) => {
	upload(req, res, async function (err) {
		...
		const gallery = await Slider.insertMany(imagesInfoData);
...
exports.getSlider = catchAsync(async (req, res, next) => {
	...
	const slider = await Slider.find(find).sort({ _id: -1 }).limit(limit).skip(startIndex);
	const totalPages = Math.ceil(total / limit);

	if (page > totalPages) return next(new AppError('Page does not exists.', 404));
	if (slider.length < 1) return next(new AppError('No photos were found.', 404));
...
exports.deleteSlider = catchAsync(async (req, res, next) => {
	...
	const slider = await Slider.findOneAndDelete({ slug });
	if (!slider) return next(new AppError(`Document not found with name: ${slug}`));
